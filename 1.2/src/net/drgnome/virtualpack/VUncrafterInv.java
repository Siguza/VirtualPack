...
public class VUncrafterInv extends VInv
{
    ...
    public void setItem(int slot, ItemStack item)
    {
        if((item != null) && (slot < 9))
        {
            List list = CraftingManager.getInstance().getRecipies(); // Derpnote: "getRecipes"
            Object tmp;
            Object tmp2;
            CraftingRecipe recipe; // Derpnote: "IRecipe"
            ItemStack result;
            for(int i = 0; i < list.size(); i++)
            {
                tmp = list.get(i);
                if(!(tmp instanceof CraftingRecipe)) // Derpnote: "IRecipe"
                {
                    continue;
                }
                recipe = (CraftingRecipe)tmp; // Derpnote: "IRecipe"
                if(recipe == null)
                {
                    continue;
                }
                ...
                for(; item.count >= result.count; item.count -= result.count)
                {
                    ...
                    for(int j = 0; j < back.length; j++)
                    {
                        if((back[j] == null) || (Item.byId[back[j].id].k())) // Derpnote
                        {
                            continue;
                        }
                        success = false;
